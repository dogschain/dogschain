...
import (
	...
	"github.com/dogschain/dogschain/x/evm"
	"github.com/dogschain/dogschain/x/evm/keeper"
	"github.com/dogschain/dogschain/x/evm/types"
...
func (suite *EvmTestSuite) SetupTest() {
	...
	suite.ctx = suite.app.BaseApp.NewContext(checkTx, abci.Header{Height: 1, ChainID: "dogschain-3", Time: time.Now().UTC()})
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				chainID, err := dogschain.ParseChainID(suite.ctx.ChainID())
				suite.Require().NoError(err)
...
func (suite *EvmTestSuite) TestMsgDogsChain() {
	var (
		tx   types.MsgDogsChain
	...
	testCases := []struct {
	...
	}{
		{
			"passed",
			func() {
				tx = types.NewMsgDogsChain(0, &to, sdk.NewInt(1), 100000, sdk.NewInt(2), []byte("test"), from)
