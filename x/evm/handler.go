...
// NewHandler returns a handler for DogsChain type messages.
func NewHandler(k *Keeper) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) (result *sdk.Result, err error) {
		...
		// See https://github.com/dogschain/dogschain/issues/668 for more information.
		defer func() {
			if r := recover(); r != nil {
		...
		case types.MsgDogsChain:
			result, err = handleMsgDogsChain(ctx, k, msg)
		default:
...
// handleMsgDogsChain handles an sdk.StdTx for an Ethereum state transition
func handleMsgDogsChain(ctx sdk.Context, k *Keeper, msg types.MsgDogsChain) (*sdk.Result, error) {
	// parse the chainID from a string to a base-10 integer
	chainIDEpoch, err := dogschain.ParseChainID(ctx.ChainID())
	if err != nil {
		return nil, err
	}
