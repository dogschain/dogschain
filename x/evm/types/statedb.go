...
// StorageTrie returns nil as the state in DogsChain does not use a direct
// storage trie.
...
// ops. It is not used in DogsChain, so it returns nil.
...
func (csdb *CommitStateDB) UpdateAccounts() {
	for _, stateEntry := range csdb.stateObjects {
		...
		dogschainAcc, ok := currAcc.(*dogschain.EthAccount)
		if !ok {
			continue
		}
		...
		balance := sdk.Coin{
			Denom:  evmDenom,
			Amount: dogschainAcc.GetCoins().AmountOf(evmDenom),
		}

		if stateEntry.stateObject.Balance() != balance.Amount.BigInt() && balance.IsValid() ||
			stateEntry.stateObject.Nonce() != dogschainAcc.GetSequence() {
			stateEntry.stateObject.account = dogschainAcc
		}
	}
}
