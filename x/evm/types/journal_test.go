...
func (suite *JournalTestSuite) SetupTest() {
	...
	balance := sdk.NewCoins(dogschain.NewPhotonCoin(sdk.NewInt(100)))
	acc := &dogschain.EthAccount{
...
// setup here instead of the DogsChain app test setup because the journal methods are private and using
...
func (suite *JournalTestSuite) setup() {
	...
	ak := auth.NewAccountKeeper(cdc, authKey, authSubspace, dogschain.ProtoAccount)
	suite.ctx = sdk.NewContext(cms, abci.Header{ChainID: "dogschain-8"}, false, tmlog.NewNopLogger())
