...
type stateObject struct {
	code dogschain.Code // contract bytecode, which gets set when code is loaded
...
func newStateObject(db *CommitStateDB, accProto authexported.Account) *stateObject {
	...
	dogschainAccount, ok := accProto.(*dogschain.EthAccount)
	if !ok {
		panic(fmt.Sprintf("invalid account type for state object: %T", accProto))
	}

	// set empty code hash
	if dogschainAccount.CodeHash == nil {
		dogschainAccount.CodeHash = emptyCodeHash
	}

	return &stateObject{
		stateDB:                 db,
		account:                 dogschainAccount,
		address:                 dogschainAccount.EthAddress(),
...
func (so *stateObject) commitState() {
	...
	for _, state := range so.dirtyStorage {
		...
		if dogschain.IsEmptyHash(state.Value) {
