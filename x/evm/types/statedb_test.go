...
func (suite *StateDBTestSuite) SetupTest() {
	...
	suite.ctx = suite.app.BaseApp.NewContext(checkTx, abci.Header{Height: 1, ChainID: "dogschain-1"})
	...
	balance := sdk.NewCoins(dogschain.NewPhotonCoin(sdk.ZeroInt()))
...
func (suite *StateDBTestSuite) TestCommitStateDB_Commit() {
	...
	for _, tc := range testCase {
		...
		ethAcc, ok := acc.(*dogschain.EthAccount)
		suite.Require().True(ok)
