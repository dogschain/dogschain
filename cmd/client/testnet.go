...
func TestnetCmd(ctx *server.Context, cdc *codec.Codec,
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a DogsChain testnet",
		Long: `testnet will create "v" number of directories and populate each with
necessary files (private validator, genesis, config, etc.).

Note, strict routability for addresses is turned off in the config file.`,

		Example: "dogschain testnet --v 4 --keyring-backend test --output-dir ./output --starting-ip-address 192.168.10.2",
	...
	cmd.Flags().String(flagNodeDaemonHome, "dogschaind", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagNodeCLIHome, "dogschaincli", "Home directory of the node's cli configuration")
...
func InitTestnet(
	...
	if chainID == "" {
		chainID = fmt.Sprintf("dogschain-%d", tmrand.Int63n(9999999999999)+1)
	}

	if !dogschain.IsValidChainID(chainID) {
	...
	for i := 0; i < numValidators; i++ {
		...
		genAccounts = append(genAccounts, dogschain.EthAccount{
