...
import (
	...
	"github.com/dogschain/dogschain/app"
	"github.com/dogschain/dogschain/app/codec"
	"github.com/dogschain/dogschain/app/crypto/ethsecp256k1"
	dogschain "github.com/dogschain/dogschain/app/types"
	"github.com/dogschain/dogschain/cmd/client"
)
...
func main() {
	...
	dogschain.SetBech32Prefixes(config)
	dogschain.SetBip44CoinType(config)
	config.Seal()
...
func exportAppStateAndTMValidators(
	...
	var dogschainApp *app.DogsChainApp

	if height != -1 {
		dogschainApp = app.NewDogsChainApp(logger, db, traceStore, false, map[int64]bool{}, 0)

		if err := dogschainApp.LoadHeight(height); err != nil {
			return nil, nil, err
		}
	} else {
		dogschainApp = app.NewDogsChainApp(logger, db, traceStore, true, map[int64]bool{}, 0)
	}

	return dogschainApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
