...
import (
	...
	"github.com/dogschain/dogschain/app/crypto/ethsecp256k1"
	dogschain "github.com/dogschain/dogschain/app/types"
)

func TestDogsChainKeygenFunc(t *testing.T) {
	...
	for _, tc := range testCases {
		privkey, err := DogsChainKeygenFunc(tc.privKey, tc.algo)
		if tc.expPass {
...
func TestKeyring(t *testing.T) {
	...
	kr, err := keys.NewKeyring("dogschain", keys.BackendTest, dir, mockIn, EthSecp256k1Options()...)
	require.NoError(t, err)
	...
	info, mnemonic, err := kr.CreateMnemonic("foo", keys.English, dogschain.BIP44HDPath, EthSecp256k1)
	require.NoError(t, err)
	...
	params := *hd.NewFundraiserParams(0, dogschain.Bip44CoinType, 0)
...
func TestDerivation(t *testing.T) {
	...
	bz, err := DeriveSecp256k1(mnemonic, keys.DefaultBIP39Passphrase, dogschain.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	privkey, err := DogsChainKeygenFunc(bz, EthSecp256k1)
	require.NoError(t, err)
	require.NotEmpty(t, privkey)

	badPrivKey, err := DogsChainKeygenFunc(badBz, EthSecp256k1)
	require.NoError(t, err)
	...
	path := hdwallet.MustParseDerivationPath(dogschain.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of DogsChain implementation
	...
	// Equality of Eth and DogsChain implementation
	...
	// Inequality of wrong derivation path of Eth and DogsChain implementation
