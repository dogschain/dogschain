...
var (
	...
	regexSeparator   = `-{1}`
	regexEpoch       = `[1-9][0-9]*`
	dogschainChainID = regexp.MustCompile(fmt.Sprintf(`^(%s)%s(%s)$`, regexChainID, regexSeparator, regexEpoch))
)
...
func IsValidChainID(chainID string) bool {
	...
	return dogschainChainID.MatchString(chainID)
}
...
func ParseChainID(chainID string) (*big.Int, error) {
	...
	matches := dogschainChainID.FindStringSubmatch(chainID)
...
func GenerateRandomChainID() string {
	return fmt.Sprintf("dogschain-%d", 10+tmrand.Intn(10000))
}
