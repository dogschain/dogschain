...
import (
	...
	"github.com/dogschain/dogschain/app/crypto/hd"
	"github.com/dogschain/dogschain/app/rpc/backend"
	rpctypes "github.com/dogschain/dogschain/app/rpc/types"
	dogschain "github.com/dogschain/dogschain/app/types"
	"github.com/dogschain/dogschain/app/utils"
...
func NewAPI(
	...
	epoch, err := dogschain.ParseChainID(clientCtx.ChainID)
	if err != nil {
		panic(err)
	}
...
func (api *PublicEthereumAPI) ProtocolVersion() hexutil.Uint {
	...
	return hexutil.Uint(dogschain.ProtocolVersion)
}
...
func (api *PublicEthereumAPI) doCall(
	...
	gas := uint64(dogschain.DefaultRPCGasLimit)
	if args.Gas != nil {
		gas = uint64(*args.Gas)
	}
	...
	gasPrice := new(big.Int).SetUint64(dogschain.DefaultGasPrice)
	...
	msg := evmtypes.NewMsgDogsChain(nonce, &toAddr, sdk.NewIntFromBigInt(value), gas,
		sdk.NewIntFromBigInt(gasPrice), data, sdk.AccAddress(addr.Bytes()))
	msgs = append(msgs, msg)

	// convert the pending transactions into dogschain msgs
	if blockNum == rpctypes.PendingBlockNumber {
...
// those transactions into dogschain messages.
func (api *PublicEthereumAPI) pendingMsgs() ([]sdk.Msg, error) {
	...
	for _, pendingTx := range pendingTxs {
		...
		pendingGasPrice := new(big.Int).SetUint64(dogschain.DefaultGasPrice)
		...
		msg := evmtypes.NewMsgDogsChain(nonce, &pendingTo, sdk.NewIntFromBigInt(pendingValue), pendingGas,
