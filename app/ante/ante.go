...
// AccountSetupDecorator sets an account to state if it's not stored already. This only applies for MsgDogsChain.
...
// AnteHandle sets an account for MsgDogsChain (evm) if the sender is registered.
...
func (asd AccountSetupDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for _, msg := range msgs {
		if msgDogsChain, ok := msg.(evmtypes.MsgDogsChain); ok {
			setupAccount(asd.ak, ctx, msgDogsChain.From)
		}
	}
